...
<div class="container" style="max-width: 1200px; margin: 20px auto; padding: 20px; background: #ffffff; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);">
    <div class="section-header" style="text-align: center;">
        <h1 style="font-size: 28px; font-weight: bold; color: #2c3e50; margin-bottom: 10px;">Detalles de Rentas</h1>
    </div>

    <!-- Información general de rentas -->
    <div class="info-box" style="margin: 20px 0; padding: 15px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9;">
        <div class="info-item" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
            ...
            <span class="info-value" style="color: #2c3e50;">Rentas</span>
        </div>
        <div class="info-item" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
    ...
    <div class="table-container" style="margin-top: 20px;">
        ...
        <form action="{{ route('rentas.store', ['obraId' => $obraId]) }}" method="POST" enctype="multipart/form-data">
            @csrf
...
<script>
    ...
    function removeRow(button) {
        ...
        fetch(`/rentas/{{ $obraId }}/detalles/${detalleId}`, {
            method: 'POST',
            headers: {
    ...
    function updateTotal() {
        ...
        fetch(`{{ route('updateCostoIndirecto', ['obraId' => $obraId, 'costo' => 'rentas']) }}?costo=${total.toFixed(2)}`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            }
