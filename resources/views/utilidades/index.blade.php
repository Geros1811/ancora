        ...
        <form action="{{ route('utilidades.store', ['obraId' => $obraId]) }}" method="POST" enctype="multipart/form-data">
            ...
            @if(Auth::check() && Auth::user()->role != 'maestro_obra' && Auth::user()->role != 'residente')
                ...
                @if(Auth::check() && Auth::user()->hasRole('arquitecto'))
                    ...
                    <a href="{{ route('utilidades.pdf', ['obraId' => $obraId]) }}" class="btn btn-primary" style="margin-top: 10px; margin-left: 10px;" target="_blank">
...
<script>
    ...
    function removeRow(button) {
        ...
        fetch(`/utilidades/{{ $obraId }}/detalles/${detalleId}`, {
            method: 'POST',
            headers: {
