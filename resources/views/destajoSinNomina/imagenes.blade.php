...
<div class="container" style="max-width: 1200px; margin: 20px auto; padding: 20px; background: #ffffff; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);">
    <div class="section-header" style="text-align: center;">
        <h1 style="font-size: 28px; font-weight: bold; color: #2c3e50; margin-bottom: 5px;">
            Imágenes de Destajo Sin Nomina
        </h1>
    </div>
        ...
        <div class="row">
            @if(isset($imagenes) && count($imagenes) > 0)
                @foreach($imagenes as $imagen)
                    <div class="col-md-3">
                        <div class="thumbnail">
<img src="{{ asset($imagen->path) }}" alt="Imagen" style="width:50%">
                            <div class="caption">
                ...
                @endforeach
            @else
                <p>No hay imágenes subidas para este destajo sin nomina.</p>
            @endif
        </div>
    </div>

    <a href="{{ route('destajosSinNomina.index', ['obraId' => $obraId]) }}" class="btn btn-primary">
        <i class="fas fa-arrow-left"></i> Volver a Destajos Sin Nomina
    </a>
</div>
