...
<div class="container" style="max-width: 1200px; margin: 20px auto; padding: 20px; background: #ffffff; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);">
    <div class="section-header" style="text-align: center;">
        <h1 style="font-size: 28px; font-weight: bold; color: #2c3e50; margin-bottom: 10px;">Detalles de IMSS</h1>
    </div>

    <!-- Información general de IMSS -->
    <div class="info-box" style="margin: 20px 0; padding: 15px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9;">
        <div class="info-item" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
            ...
            <span class="info-value" style="color: #2c3e50;">IMSS</span>
        </div>
        <div class="info-item" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
    ...
    <div class="table-container" style="margin-top: 20px;">
        ...
        <form action="{{ route('imss.store', ['obraId' => $obraId]) }}" method="POST">
            @csrf
...
<td style="border: 1px solid #ddd; text-align: center; padding: 10px;"><input type="number" name="importe[]" value="{{ $detalle->importe }}" step="0.01" class="form-control importe" style="border: none; background: transparent; text-align: center;" oninput="updateSubtotal(this)"></td>
    ...
    <div class="actions" style="margin-top: 20px; text-align: center;">
        <a href="{{ route('imss.index', ['obraId' => $obraId]) }}" class="btn btn-primary" style="display: inline-block; background-color: #007bff; color: white; text-decoration: none; padding: 10px 20px; border-radius: 5px; transition: background-color 0.3s ease;">Regresar</a>
    </div>
</div>
...
<script>
    function addRow() {
        ...
        newRow.innerHTML = `
            ...
            <td style="border: 1px solid #ddd; text-align: center; padding: 10px;"><input type="number" name="importe[]" step="0.01" class="form-control importe" style="border: none; background: transparent; text-align: center;" oninput="updateSubtotal(this)"></td>
    ...
    function updateTotal() {
        ...
        fetch(`{{ route('updateCostoIndirecto', ['obraId' => $obraId, 'costo' => 'imss']) }}?costo=${total.toFixed(2)}`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            }
