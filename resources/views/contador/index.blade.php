...
<div class="container" style="max-width: 1200px; margin: 20px auto; padding: 20px; background: #ffffff; border-radius: 10px; box-shadow: 0 4px 10px rgba(0, 0, 0, 0.1);">
    <div class="section-header" style="text-align: center;">
        <h1 style="font-size: 28px; font-weight: bold; color: #2c3e50; margin-bottom: 10px;">Detalles de Contador</h1>
    </div>

    <!-- Información general de Contador -->
    <div class="info-box" style="margin: 20px 0; padding: 15px; border: 1px solid #ddd; border-radius: 5px; background-color: #f9f9f9;">
        <div class="info-item" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
            ...
            <span class="info-value" style="color: #2c3e50;">Contador</span>
        </div>
        <div class="info-item" style="display: flex; justify-content: space-between; margin-bottom: 10px;">
    ...
    <div class="table-container" style="margin-top: 20px;">
        ...
        <form action="{{ route('contador.store', ['obraId' => $obraId]) }}" method="POST">
            @csrf
    ...
    <div class="actions" style="margin-top: 20px; text-align: center;">
        <a href="{{ route('contador.index', ['obraId' => $obraId]) }}" class="btn btn-primary" style="display: inline-block; background-color: #007bff; color: white; text-decoration: none; padding: 10px 20px; border-radius: 5px; transition: background-color 0.3s ease;">Regresar</a>
    </div>
</div>
...
function removeRow(button, detalleId) {
    ...
    function updateTotal() {
        ...
        // Actualizar el costo total en la vista de pagos administrativos
        fetch(`{{ route('updateCostoIndirecto', ['obraId' => $obraId, 'costo' => 'contador']) }}?costo=${total.toFixed(2)}`, {
            method: 'POST',
            headers: {
                'X-CSRF-TOKEN': '{{ csrf_token() }}'
            }
