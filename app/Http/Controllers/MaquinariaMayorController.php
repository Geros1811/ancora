...
class MaquinariaMayorController extends Controller
{
    public function index($obraId)
    {
        $detalles = DetalleMaquinariaMayor::where('obra_id', $obraId)->get();
        $costoTotal = $detalles->sum('subtotal');
        $obra = Obra::findOrFail($obraId);
        return view('maquinariaMayor.index', compact('detalles', 'obraId', 'costoTotal', 'obra'));
    }
    ...
    public function store(Request $request, $obraId)
    {
        ...
        foreach ($fechas as $index => $fechaInput) {
            ...
            if (isset($ids[$index])) {
                $detalle = DetalleMaquinariaMayor::find($ids[$index]);
                if (!$detalle) {
                    $detalle = new DetalleMaquinariaMayor();
                }
            } else {
                $detalle = new DetalleMaquinariaMayor();
            }
        ...
        CostoDirecto::updateOrCreate(
            ['obra_id' => $obraId, 'nombre' => 'Maquinaria Mayor'],
            ['costo' => $costoTotal]
        );

        return redirect()->route('maquinariaMayor.index', ['obraId' => $obraId]);
    }

    public function destroy($id)
    {
        $detalle = DetalleMaquinariaMayor::findOrFail($id);
        $detalle->delete();
