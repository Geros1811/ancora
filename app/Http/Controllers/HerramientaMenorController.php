...
class HerramientaMenorController extends Controller
{
    public function index($obraId)
    {
        ...
        return view('herramientaMenor.index', compact('detalles', 'obraId', 'costoTotal', 'obra'));
    }
    ...
    public function store(Request $request, $obraId)
    {
        ...
        foreach ($fechas as $index => $fechaInput) {
            ...
            if (isset($ids[$index])) {
                $detalle = DetalleHerramientaMenor::find($ids[$index]);
                if (!$detalle) {
                    $detalle = new DetalleHerramientaMenor();
                }
            } else {
                $detalle = new DetalleHerramientaMenor();
            }
        ...
        CostoDirecto::updateOrCreate(
            ['obra_id' => $obraId, 'nombre' => 'Herramienta Menor'],
            ['costo' => $costoTotal]
        );

        return redirect()->route('herramientaMenor.index', ['obraId' => $obraId]);
    }

    public function destroy($id)
    {
        $detalle = DetalleHerramientaMenor::findOrFail($id);
        $detalle->delete();
